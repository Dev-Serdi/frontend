...
import {
  listAllUbicaciones,
} from "../../../services/UbicacionService"; // Asegúrate de tener deletePrioridad
...
export const Ubicaciones = () => {
  ...
  const filteredUbicaciones = ubicaciones.filter((ubicaciones) => {
    const term = searchTerm.toLowerCase();
    const nombre = ubicaciones.nombre
      ? String(ubicaciones.nombre).toLowerCase()
      : "";
    return nombre.includes(term);
  });

  // Calcular ubicaciones para la página actual
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = filteredUbicaciones.slice(
    indexOfFirstItem,
    indexOfLastItem
  ); // Usar ubicaciones filtradas
  const totalPages = Math.ceil(filteredUbicaciones.length / itemsPerPage); // Usar longitud de ubicaciones filtradas
  ...
  const deleteHandler = async (ubicacionidId) => {
    if (
      window.confirm("¿Estás seguro de que deseas eliminar esta ubicacion?")
    ) {
      try {
        setLoading(true);
        // await deleteUbicacion(ubicacionidId); // Llamar al servicio de eliminación
        fetchUbicaciones(); // Recargar la lista
      } catch (err) {
        setError("Error al eliminar ubicaciones");
        console.error(err);
      } finally {
        setLoading(false);
      }
...
s                        onClick={() =>
                          navigate(`/admin/helpdesk/ubicaciones/editar/${ubicaciones.id}`)
                        }
                      ...
                      {/* <button
                        ...
                        onClick={() => deleteHandler(ubicaciones.id)} // Pasar el id de la ubicaciones
