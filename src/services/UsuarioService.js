...
export const listUsers = (departamento,currentPage, itemsPerPage, searchTerm) => {
  if (departamento) {
    return axios
      .get(
  ...
  return axios
    .get(`${REST_API_BASE_URL}/users?page=${currentPage}&size=${itemsPerPage}&searchTerm=${searchTerm}`, getHeaders()) // Add headers to the request
    .then((response) => response)
    .catch((error) => {
      if (!error.response) {
        throw new Error("Error de conexion con el servidor");
